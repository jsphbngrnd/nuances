...
function MatchmakingPageInner() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setMatched(true), 4200);
    return () => { clearInterval(tick); clearTimeout(t); };
  }, []);

  useEffect(() => {
    if (!matched) return;
    const t = setTimeout(() => router.push(`/topic?mode=${mode}`), 1500);
    ...
    <div className="nuance-phone">
      ...
      <Screen scroll={false} style={{ display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
          ...
          <p className="np-eyebrow" style={{ marginTop: 30 }}>{matched ? "Match found" : "Searching"}</p>
          ...
          <p style={{ margin: "14px auto 0", fontSize: 12.5, lineHeight: 1.45, color: "var(--muted)", maxWidth: "28ch" }}>
            {matched
              ? "Trusted, ready to talk, in their own language. Bringing you both to the topic."
              : "Matching on mode and trust score. Language is no barrier — NUANCE translates every message in real time."}
          </p>
...
export default function MatchmakingPage() { return <Suspense fallback={<div className="nuance-phone" />}><MatchmakingPageInner /></Suspense>; }
